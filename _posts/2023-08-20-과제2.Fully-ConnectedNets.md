...
<div>
    Answer : 만약 계층이 더 깊다면 많은 가중치를 학습시켜야 한다. 즉 계층이 많으면 많을 수록과 과접합과 관련이 되기 쉽습니다.<br>
    가중치는 초깃값을 작은 값으로 설정하게 된다면, 레이가 얇은 (3 계층)는 forward 진행되고 output 레이어까지 문제 없이 진행되지만, 레이어가 깊으면 깊을 수 록, 작은 값을 곱하게 된다면 W 값들이 0으로 수렴하게 됩니다. 즉 더 깊은 계층이 가중치에 민감하게 반응하게 됩니다.
</div>
