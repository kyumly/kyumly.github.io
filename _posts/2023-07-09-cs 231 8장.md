...
To see this, you can use Taylor expansion of f(x+h) and f(x−h) and verify that the first formula has an error on order of O(h), while the second formula only has error terms on order of O(h2)  (i.e. it is a second order approximation).<br>
- 이 경우 기울기의 모든 차원을 확인하기 위해 손실 함수를 두 번 평가해야 하지만(따라서 비용이 약 2배) 기울기 근사치가 훨씬 정확한 것으로 나타났습니다.
- 이를 확인하려면 f(x+h)와 f(x-h)의 Taylor 확장을 사용하여 첫 번째 공식에 O(h) 순서의 오차가 있는지 확인하고 두 번째 공식에는 O(h2) 순서의 오차항만 있는지 확인할 수 있습니다(즉, 2차 근사).



Use relative error for the comparison.<br>
...
- 1e-4 > 상대 오차는 일반적으로 꼬임이 있는 목표에 대해 괜찮습니다.
- 그러나 꼬임(예: 탄 비선형성 및 소프트맥스 사용)이 없으면 1e-4가 너무 높습니다.
- 1e-7 이하는 행복해야 합니다.
...
**Use double precision.** A common pitfall is using single precision floating point to compute gradient check.<br>
...
**Stick around active range of floating point.** <br>
...
**Kinks in the objective.** One source of inaccuracy to be aware of during gradient checking is the problem of kinks.<br>
...
- 꺽임은 ReLU(max(0,x)) 또는 SVM 손실, Maxout 뉴런 등과 같은 함수에 의해 도입된 목적 함수의 미분할 수 없는 부분을 나타냅니다.예를들어 x=−1e6에서 ReLU 함수의 그래디언트 검사를 고려하십시오.
...
- f(x+h)를 평가한 다음 f(x-h)를 평가할 때 적어도 하나의 승자의 ID가 변경되면 꼬임이 교차되고 수치 기울기가 정확하지 않게 됩니다.

**Use only few datapoints.**<br>
...
Moreover, if your gradcheck for only ~2 or 3 datapoints then you would almost certainly gradcheck for an entire batch.<br>
Using very few datapoints also makes your gradient check faster and more efficient.<br>

- 작은 수의 데이터만 사용
- loss function이 꺽임을 포함하는 경우(예를 들어, ReLU 또는 margin losses 등을 사용할 때), 더 적은 데이터포인트를 사용하면 더 적은 꺽임를 가지기 때문에, 유한 차분 근사(infinite difference approximation)를 수행할 때 이를 건너뛸(cross) 가능성이 낮아지기 때문입니다
- 게다가 ~2개 또는 3개의 데이터 포인트에 대해서만 그라데이션 검사를 수행하면 전체 배치에 대해 그라데이션 검사를 수행할 수 있습니다.
- 또한 매우 적은 수의 데이터 포인트를 사용하면 그라데이션 검사를 더 빠르고 효율적으로 수행할 수 있습니다.



**Be careful with the step size h.**<br>
...
Gradcheck during a “characteristic” mode of operation.<br>
...
Don’t let the regularization overwhelm the data. <br>
...
Remember to turn off dropout/augmentations.<br>
