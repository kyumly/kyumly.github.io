...
For example, we can interpret σ(∑iwixi+b) to be the probability of one of the classes P(yi=1∣xi;w).<br>
The probability of the other class would be P(yi=0∣xi;w)=1−P(yi=1∣xi;w), since they must sum to one.<br>
...
- 이진 Softmax 분류기. 예를 들어, σ(∑iwixi+b)를 클래스 P(yi=1∣xi;w) 중 하나의 확률로 해석할 수 있습니다.
- 다른 클래스의 확률은 합이 1이어야 하므로 P(yi=0∣xi;w)=1−P(yi=1∣xi;w)입니다. 
...
### 2.3.1 Sigmoid.
The sigmoid non-linearity has the mathematical **form σ(x)=1/(1+e−x)** and is shown in the image above on the left. 
...
- 시그모이드
- 시그모이드 비선형성 수학적 형식은 σ(x)=1/(1+e−x)이며 위의 왼쪽 이미지에 표시되어 있습니다. 
